...
public interface BoardRepository extends JpaRepository<Board, Long> {
    ...
    @Query("SELECT b FROM Board b LEFT JOIN b.user u WHERE b.hide = false AND u.role = :role")
    Page<Board> findAllByRole(@Param("role") Role role, Pageable pageable);

    @Query("SELECT b FROM Board b LEFT JOIN b.user u WHERE b.hide = false AND (b.title LIKE %:keyword% OR b.content LIKE %:keyword% OR u.nickName LIKE %:keyword%) AND u.role = :role")
